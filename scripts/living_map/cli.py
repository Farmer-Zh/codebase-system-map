"""Command-line interface for RepoAtlas."""
...
def parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(prog="repo-atlas", description="Turn a code repository into a product-readable system map.")
    ...
    root.add_argument("--work-dir", type=Path, default=Path(".repo-atlas"), help="CodeWiki cache/database directory")
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Keep the earlier ``living-map build <repo>`` spelling as a compatibility
    # alias while making the normal interface just ``repo-atlas <repo>``.
